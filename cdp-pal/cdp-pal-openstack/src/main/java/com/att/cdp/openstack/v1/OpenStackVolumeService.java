...
/**
 * This class implements the OpenStack volume service when the version is V1.0, which is actually provided as part of
 * Nova.
 * <p>
...
public class OpenStackVolumeService extends AbstractVolume {
    ...
    /**
     * Create the V1.0 volume service. This service uses the Nova (compute) service since this version of OpenStack does
     * not define the break-out to the Cinder service yet.
     * 
     * @param context
    ...
    public Volume createVolume(Volume template) throws ZoneException {
        ...
        try {
            com.woorea.openstack.nova.model.VolumeForCreate newVolume =
                new com.woorea.openstack.nova.model.VolumeForCreate();
    ...
    public Snapshot createSnapshot(Snapshot template) throws ZoneException {
        ...
        try {
            com.woorea.openstack.nova.model.SnapshotForCreate newSnapshot =
                new com.woorea.openstack.nova.model.SnapshotForCreate();
            HashMap<String, String> dictionary = new HashMap<>();
            dictionary.put("name", "name");
            dictionary.put("description", "description");
            dictionary.put("size", "size");
            ...
            OpenStackSnapshot snapshot =
                new OpenStackSnapshot(context, nova.getClient().snapshots().create(newSnapshot).execute());
            return snapshot;
        } catch (OpenStackBaseException ex) {
            ExceptionMapper.mapException(ex);
        }
