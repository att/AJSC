...
/**
 * A server is the definition of a virtual machine (and also represents the
 * running image of that server) that requires a specific hardware environment
 * (specified by a template), boots from a specific disk image (an Image), is a
 * member of a specific tenant (the Tenant), and may be accessed over one or
 * more subnets connected to ports (NICs).
 * <p>
 * The server class can also be used to create a new server. In that case, the
 * server is instantiated and initialized with a name, template, an image, and
 * optionally an ACL name). The server is then used as a prototype to create the
...
public class Server extends ModelObject {
	...
	/**
	 * The status of the server. This status is a composite view of the server.
	 * Different implementations may report status differently, and may report
	 * multiple status values of various sub-systems. The API implementation
	 * must aggregate these status values into a single status defined in this
	 * enumeration, and which represents the server definition and/or instance
	...
	public enum Status {
		...
		/**
		 * The server definition has been deleted. Not all implementations may
		 * support this state. If this state is reported, the server may not be
		 * returned to any other state.
		 */
		DELETED,

		/**
		 * The server is in an error state and cannot be used. It may be
		 * returned to the pending state if there are modifications made, or to
		 * the running state if it simply failed and has been restarted.
		 */
		ERROR,

		/**
		 * The server is being built or rebuilt, or it is in some state of flux
		 * and cannot be operated upon. It is not active nor can it currently be
		 * used. This state applies to any condition where the server is not
		 * usable, possibly because of structural changes being made, such as
		 * resizing.
		 */
		PENDING,

		/**
		 * The server is ready for use but is not currently running anywhere.
		 * This state indicates that the definition is valid, it can be
		 * manipulated or changed, and it can be started but is currently not
		 * running.
		 */
		READY,
		...
		/**
		 * The server was running but has been suspended. This means that the
		 * virtual machine has been swapped to disk and is not currently in a
		 * runnable state, but the VM still exists and can be resumed at some
		 * point in time. Not all implementations may support suspend/resume.
		 */
		SUSPENDED,

		/**
		 * The server was running but was paused. A paused server is simply
		 * frozen and not dispatchable on the hypervisor. It can be unpaused and
		 * continued. It is NOT swapped to disk and technically is still in a
		 * runnable state, just not being actively dispatched. This is a
		 * variation of suspended that some IaaS providers may offer. Not all
		 * implementations may support pause/unpause.
		 */
		PAUSED;
	}
	...
	/**
	 * The list of access control lists (ACL's) assigned to this server to
	 * control access.
	 */
	...
	/**
	 * The fault descriptor if there is a problem with this server. It will be
	 * null if there is no fault.
	 */
	private Fault fault;
	 ...
	 * 
	 * @Deprecated use {@link Port} to obtain IP addresses for a specific NIC
	 *             card on a specific subnet
	 */
	@Deprecated
	...
	/**
	 * The name of the server. Typically, this is how the server is seen in any
	 * consoles or user interfaces.
	 */
	private String name;

	/**
	 * The list of networks that this server is attached to (virtual NIC
	 * interfaces)
	 * 
	...
	/**
	 * The list of ports to be attached to the server when it is created. This
	 * list is not used after the creation of the server.
	 */
	private List<Port> ports;

	/**
	 * A list of snapshots, in chronological order (newest first) that were
	 * created from this server, or an empty list if no snapshots exist.
	 */
	private List<Image> snapshots = new ArrayList<>();

	/**
	 * The timestamp (expressed as a String) when the vm was started. Note, the
	 * specific provider may or may not have this information available. If not
	 * available, it will be null.
	 */
	private String started;

	/**
	 * The status of the server. This status reflects the condition of the
	 * definition as well as its running state, if any.
	 */
	private Status status;

	/**
	 * The timestamp (expressed as a String) when the vm was stopped. Note, the
	 * specific provider may or may not have this information available. If not
	 * available, it will be null.
	 */
	...
	/**
	 * A mapping of all volumes attached to the server, keyed by the device
	 * name.
	 */
	...
	/**
	 * This protected constructor allows the creation of a connected model
	 * object that is connected to a specific context
	 * 
	 * @param context
	...
	/**
	 * This constructor can be used to create a Server object suitable for
	 * requesting the creation of the host.
	 * 
	 * @param name
	...
	/**
	 * Creates a template server object suitable for creating a new server that
	 * specifies the name, image, template, and access control list(s) to be
	 ...
	 *            The reference to the Template id to be used
	 * @param aclName
	 *            The name of the access control list that we want to associate
	 *            with the new server.
	 */
	 ...
	 * @deprecated use {@link #addPort(Port)} instead.
	 */
	@Deprecated
	 ...
	 * 
	 * @param pool
	 *            The name of the floating IP address pool to reserve the
	 *            address from
	 * @return The assigned IP address
	 * @throws ZoneException
	 *             If the address cannot be assigned
	...
	/**
	 * @return A map of volume attachments to the server. The map is keyed by
	 *         the device name. The value of the map is the id of the volume.
	 * @throws ZoneException
	 ...
	 * 
	 * @return One of the enumerated constants indicating the source of the boot
	 *         for the server
	 */
	...
	/**
	 * Returns the list of all fixed ip addresses assigned to this server, if
	 * any
	 * 
	 * @return A list of all fixed ip addresses, if any, assigned to this
	 *         server. If none are assigned, the list will be empty.
	 */
	 ...
	 * 
	 * @return The list of network names. Note that the list may be empty if not
	 *         networks have been assigned
	 * @Deprecated
	 */
	@JsonIgnore
	@Deprecated
	...
	/**
	 * @return The map of all volumes attached to this server keyed by the
	...
	/**
	 * This method rebuilds the VM using the exact same template (flavor) and
	 * image that it was created from. It does not take into consideration if
	 * there are any snapshots available or not.
	 * 
	 * @throws ZoneException
	...
	/**
	 * This method is used to rebuild a VM from a snapshot of the VM. The
	 * snapshot must be associated with the VM being rebuilt.
	 * 
	 * @param snapshot
	...
	/**
	 * This method informs the provider implementation to refresh this server
	...
	/**
	 * This method should be used instead of {@link #refresh()} to refresh the
	 * entire server model. If it is only needed to refresh the server status,
	 * use {@link #refreshStatus()} instead.
	 * <p>
	 * This method discards the entire model of the server, as well as all
	 * related objects, and rebuilds the entire model of the server.
	 * </p>
	 * 
	 * @throws ZoneException
	 *             If the server cannot be refreshed
	 ...
	 * 
	 * @param address
	 *            The address to be released from this server and returned to
	 *            the pool
	 * @throws ZoneException
	...
	/**
	 * This method returns the ports attached to the server in the case of an
	 * existing server, or the set of ports to be attached to the server in the
	 * case of a server being created.
	 * <p>
	 * Once a server exists, the list of ports is NOT maintained in local state.
	 * Obtaining the list of ports of an existing server is done by querying the
	 * provider. This is necessary to allow the server to change independently
	 * of this abstract data model. It also allows the ports to be obtained
	 * lazily, that is, when they are needed.
	 * </p>
	 * <p>
	 * In the case of a server that is used as a model to create a new instance,
	 * the ports collection is set to the list of ports to be attached to the
	 * server. Once created, the collection is NOT maintained in the instance
	 * server object.
	 * </p>
	 * <p>
	 * When the server is connected, this method is overridden and will perform
	 * the lookup of the ports from the provider.
	 * </p>
	 * 
	 * @return A list of the virtual NIC (Network Interface Card) ports. Each
	 *         port represents a connection to a subnet.
	 * @throws ZoneException
	 ...
	 * @throws ZoneException
	 *             If the server is not connected, or if the attachment failed
	 *             for some reason
	 */
	 ...
	 * @throws ZoneException
	 *             If the server is not connected, or if the detach failed for
	 *             some reason
	 */
	...
	/**
	 * Sets the list of ports to the specified values. This is used to construct
	 * a new server with the associated port attachment(s).
	 * 
	 * @param ports
	 *            The list of ports
	 */
	...
	/**
	 * This is a standardized method to wait for the state of the server to
	 * change to one of the allowed states.
	 * <p>
	 * This method will block the caller and not return until either:
	 * <ul>
	 * <li>The server provided has been found to be in one of the provided
	 * states on a sample</li>
	 * <lI>The server does not enter any of the provided states within the
	 ...
	 * determine if the server has changed to one of the listed states. If the
	 * state does not enter one of the allowed or expected states within the
	 * timeout period, then the method throws an exception.
	 * </p>
	 * <p>
	 * The server object must be a connected server object. Using a disconnected
	 * server object is an error and will result in an exception being thrown.
	 * </p>
	 * <p>
	 * The caller provides a variable list of allowed server states. This is a
	 * variable argument list that allows for one or more states to be listed.
	 * If the server is found to be in any one of these states on a sample
	 * interval, then the method returns and no exception is thrown.
	 * </p>
	 * 
	 * @param pollInterval
	 *            The interval, in seconds, to check the server state and see if
	 *            the server state has changed.
	 * @param timeout
	 *            The total time, in seconds, that the method will block the
	 *            caller and check the server state. This value MUST be greater
	 *            than or equal to the poll interval.
	 * @param status
	 *            The variable list of at least one status value(s) that are
	 *            allowed or expected. If the server is found to be in any of
	 *            these states on a poll interval, the method completes normally
	 *            and returns to the caller.
	 * @throws TimeoutException
	 *             If the server state does not change to one of the allowed
	 ...
	 *             If the context connected to the server is closed and cannot
	 *             be used.
	 * @throws ZoneException
	...
	public String toString() {
		...
		if (isServerInError()) {
			return String.format(
					"Server: id(%s), name(%s), created(%s), updated(%s), status(%s),  "
							+ "started(%s), stopped(%s), FAULT(%s)",
					id, name, created, updated, status, started == null ? "unknown" : started,
					stopped == null ? "unknown" : stopped, fault.toString());
		}
