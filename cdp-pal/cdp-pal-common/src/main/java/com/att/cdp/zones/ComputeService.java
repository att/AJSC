...
/**
 * This interface represents the implementation of the Compute service obtained
 * from a provider context.
 * <p>
 * The compute service is used to manage compute resources, ie, servers. The
 * user can obtain lists of existing servers, create new servers, attach and
 * detach volumes, and delete servers using this service. Alternatively, the
 * rime data model can implicitly invoke these services from the various model
 * objects.
 * </p>
 * 
 * @since Sep 23, 2013
...
public interface ComputeService extends Service {
	...
	/**
	 * Forcibly associates the indicated floating IP address with the specified
	 * server.
	 * 
	 * @param server
	 *            The server object to be associated with the floating IP
	 *            address
	 * @param address
	 ...
	 * 
	 * @param serverId
	 *            The ID of the server to be associated with the floating IP
	 *            address
	 * @param address
	 *            The floating IP address to be associated with the server
	 * @throws ZoneException
	 ...
	 * @param deviceName
	 *            The device name that will be used to identify this volume,
	 *            such as /dev/sda
	 * @throws ZoneException
	 ...
	 * 
	 * @param acl
	 *            An object initialized with the contents of the ACL to be
	 *            created. This object is used as a model of what to build, but
	 *            it is likely not the object that will be built. The caller
	 *            should use the object returned from this call to access the
	 *            actually constructed ACL.
	 ...
	 * 
	 * @param server
	 *            The server to create. The host must contain the following
	 *            attributes in order to be legal for creating a new host:
	 *            <ul>
	 ...
	 * @throws ZoneException
	 *             If the access control list does not exist or cannot be
	 ...
	 * @throws ZoneException
	 *             If the rule does not exist or cannot be deleted for some
	 *             reason.
	 */
	...
	/**
	 * Executes a command on the indicated server using the console for that
	 * server
	 * 
	 * @param server
	 *            The server object
	...
	/**
	 * This method scans the provider to locate all servers (instances) that are
	 * using the specified SSH key pair, if any.
	 * 
	 * @param keyPair
	 *            The name of the key pair to search for
	 * @return A list of server id's that are using the indicated key pair. If
	 *         no servers are using the key pair, then an empty list is
	 *         returned.
	 * @throws ZoneException
	 ...
	 *         volume. The value of the entry is the volume ID of the volume
	 *         attached at that device.
	 * @throws ZoneException
	...
	/**
	 * Returns the buffered console output accumulated so far. Once the console
	 * output has been returned, it is no longer available.
	 * 
	 * @param server
	 *            The server to obtain the console output for.
	 * @return The console output. If no console has been established for this
	 *         server, then the response is null.
	 * @throws ZoneException
	 ...
	 *             <ul>
	 *             <li>the user has not successfully logged in to the provider
	 *             </li>
	 ...
	 *            <code>String.matches()</code> method. If null, the method
	 *            behaves identically to {@link #getServers()}
	 * @return The server
	 * @throws ZoneException
	 *             If the host cannot be found
	...
	/**
	 * Allows the caller to migrate a (possibly running) server from one
	 * physical host to another, where the provider controls the location and
	 * the migration of the running server.
	 * 
	 * @param serverId
	 *            The server to be migrated
	 * @throws ZoneException
	 *             If the provider does not support migrate, or the server cant
	 *             be migrated for some reason.
	 */
	void migrateServer(String serverId) throws ZoneException;

	/**
	 * The ability to move a server from whatever physical host it is located on
	 * to a new physical host, possibly supplying the new target physical host
	 * id.
	 * <p>
	 * The server to be moved must be stopped. This is likely caused by a
	 * failure of the physical host, and this method allows the instance to be
	 * moved to a new physical host. If no target host id is supplied, then the
	 * instance is moved to any suitable physical host as determined by the
	 * provider.
	 * </p>
	 * 
	 * @param serverId
	 ...
	 * @param targetHostId
	 *            The physical host id of the new host, or null to allow the
	 *            provider to control the move. This could be controlled through
	 *            policies or strategies, or other mechanisms defined by the
	 *            provider.
	 * @throws ZoneException
	 *             If the provider does not support move, or if the server cant
	 ...
	 * @throws ZoneException
	 *             If the server cannot be paused, or the provider does not
	 *             support pause
	 */
	void pauseServer(Server server) throws ZoneException;

	/**
	 * Pauses the server with the specified id, if supported by the underlying
	 * IaaS provider
	 * 
	 * @param id
	 ...
	 * @param newTemplateId
	 *            The id of the new template to be applied
	 * @throws ZoneException
	 *             If the server is in an invalid state, does not exist, or the
	 *             server object is disconnected.
	 */
	void prepareResize(Server server, String newTemplateId) throws ZoneException;
	...
	/**
	 * This method is used to rebuild a server using the exact same template
	 * (flavor) and image used to previously construct it.
	 * 
	 * @param server
	...
	/**
	 * This method is used to rebuild a server using a specified snapshot. The
	 * snapshot MUST have been created from the server being rebuilt.
	 * 
	 * @param server
	 *            The server being rebuilt
	 * @param snapshot
	 *            The snapshot image id of this server that is used to rebuild
	 *            the server.
	 * @throws ZoneException
	 */
	 ...
	 * 
	 * @param server
	 *            The server object to be refreshed. Only the status of the
	 *            server is updated, no other part of the model is changed.
	 * @throws ZoneException
	...
	/**
	 * This method releases the floating IP address from the server and returns
	 * it to the pool. The address must have been a floating ip address and it
	 * must be assigned to the specified server.
	 * 
	 * @param server
	 *            The server that the address is assigned to
	 * @param assignedAddress
	 *            The floating ip address to be released and returned to the
	 *            pool
	 * @throws ZoneException
	 ...
	 * @throws ZoneException
	 *             If the server was not paused, or the provider does not
	 *             support pause/unpause
	 */
	void unpauseServer(Server server) throws ZoneException;

	/**
	 * Un-Pauses the server with the specified id, if supported by the
	...
	/**
	 * This method obtains a list of ports (NICs) associated with the server and
	...
	/**
	 * This method can be used to attach a virtual NIC (port) to a specific
	 * server. The subnet associated with the port provides the connectivity to
	 * the server through the port.
	 * 
	 * @param server
	...
	/**
	 * This method is used to detach a specific port from a server,
	 * disconnecting it from the associated subnet.
	 * 
	 * @param server
	 ...
	 * </p>
	 * <p>
	 * The caller must also provide the server object to be checked. The server
	 * object must be a connected server object. Using a disconnected server
	 * object is an error and will result in an exception being thrown.
	 * </p>
	 * <p>
	 * The caller lastly provides a variable list of allowed server states. This
	 * is a variable argument list that allows for one or more states to be
	 * listed. If the server is found to be in any one of these states on a
